...
class SocketService {
  ...
  sendVideoCallOffer(dealId: string, targetUserId: string, offer: RTCSessionDescriptionInit) {
    this.socket?.emit('videoCallOffer', { dealId, targetUserId, offer });
  }

  sendVideoCallAnswer(dealId: string, targetUserId: string, answer: RTCSessionDescriptionInit) {
    this.socket?.emit('videoCallAnswer', { dealId, targetUserId, answer });
  }

  sendIceCandidate(dealId: string, targetUserId: string, candidate: RTCIceCandidate) {
